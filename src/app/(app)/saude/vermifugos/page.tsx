...
function PetSelectionCard({ pet }: { pet: Pet }) {
  ...
  return (
    <Link href={`/saude/vermifugos/${pet.id}`} className="block group">
...
export default function VermifugosPage() {
    ...
    <div className="container mx-auto py-8 px-4">
      <Card className="max-w-3xl mx-auto shadow-lg border-primary/20">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-3xl font-bold text-primary">Controle de Vermifugação</CardTitle>
          <CardDescription className="text-lg text-muted-foreground">
            Selecione um pet para gerenciar o histórico de vermifugação, registrar novas aplicações e visualizar o plano.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {userPets.length === 0 ? (
            <div className="text-center p-6 border-2 border-dashed rounded-lg bg-muted/20 mt-6">
              ...
              <p className="text-muted-foreground mb-4">
                Para utilizar o controle de vermifugação, primeiro adicione um perfil para seu pet.
              </p>
            ...
            <div className="text-center p-6 border rounded-md bg-muted/50 mt-6">
              ...
              <p className="text-muted-foreground">
                O controle de vermifugação só pode ser usado para pets ativos. Verifique o status dos seus pets em "Meus Pets".
              </p>
            </div>
          ) : (
          ...
          {userPets.length > 0 && activeUserPets.length < userPets.length && (
            <p className="text-sm text-muted-foreground text-center mt-4">
              Pets com status diferente de "ativo" não são listados aqui para controle de vermifugação.
            </p>
          )}
        </CardContent>
      </Card>
