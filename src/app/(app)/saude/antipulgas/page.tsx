...
function PetSelectionCard({ pet }: { pet: Pet }) {
  ...
  return (
    <Link href={`/saude/antipulgas/${pet.id}`} className="block group">
...
export default function AntipulgasPage() {
    ...
    <div className="container mx-auto py-8 px-4">
      <Card className="max-w-3xl mx-auto shadow-lg border-primary/20">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-3xl font-bold text-primary">Controle de Antipulgas e Carrapatos</CardTitle>
          <CardDescription className="text-lg text-muted-foreground">
            Selecione um pet para gerenciar o histórico de aplicações de antipulgas/carrapatos e registrar novos produtos.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {userPets.length === 0 ? (
            <div className="text-center p-6 border-2 border-dashed rounded-lg bg-muted/20 mt-6">
              ...
              <p className="text-muted-foreground mb-4">
                Para utilizar o controle de antipulgas, primeiro adicione um perfil para seu pet.
              </p>
            ...
            <div className="text-center p-6 border rounded-md bg-muted/50 mt-6">
              ...
              <p className="text-muted-foreground">
                O controle de antipulgas só pode ser usado para pets ativos. Verifique o status dos seus pets em "Meus Pets".
              </p>
            </div>
          ) : (
          ...
          {userPets.length > 0 && activeUserPets.length < userPets.length && (
            <p className="text-sm text-muted-foreground text-center mt-4">
              Pets com status diferente de "ativo" não são listados aqui para controle de antipulgas.
            </p>
          )}
        </CardContent>
      </Card>
