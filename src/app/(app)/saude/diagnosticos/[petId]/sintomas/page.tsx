...
export default function PetSintomasDiagnosticosPage() {
    ...
    if (petId && user) {
      ...
      if (foundPet) {
        if (foundPet.status.value !== 'ativo') {
            ...
            router.push("/saude/diagnosticos");
        } else {
            setPet(foundPet);
    ...
    try {
      ...
      if (result.needsMoreInfo && result.suggestedFollowUpSymptoms && result.suggestedFollowUpSymptoms.length > 0) {
        ...
        setCurrentFollowUpSymptoms([]); 
         form.reset({ symptomsDescription: data.symptomsDescription }); 
        toast({
