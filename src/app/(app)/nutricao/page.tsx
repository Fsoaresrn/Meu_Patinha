...
import { Utensils, BrainCircuit, Info } from "lucide-react";
...
function PetSelectionCard({ pet }: { pet: Pet }) {
  ...
  return (
    <Link href={`/nutricao/${pet.id}/plano`} className="block group">
    ...
    <div className="container mx-auto py-8 px-4">
      <Card className="max-w-3xl mx-auto shadow-lg border-primary/20">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-3xl font-bold text-primary">Planos Nutricionais com IA</CardTitle>
          <CardDescription className="text-lg text-muted-foreground">
            Selecione um pet para gerar um plano nutricional personalizado com auxílio da nossa Inteligência Artificial.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
           <Alert variant="default" className="bg-blue-50 dark:bg-blue-900/30 border-blue-300 dark:border-blue-700 text-blue-700 dark:text-blue-300">
            ...
            <AlertDescription>
              Os planos nutricionais gerados por IA são sugestões e <strong>não substituem</strong> o aconselhamento de um médico veterinário ou zootecnista especializado em nutrição animal. Consulte sempre um profissional para as necessidades específicas do seu pet.
            </AlertDescription>
          </Alert>
          ...
          {userPets.length === 0 ? (
            <div className="text-center p-6 border-2 border-dashed rounded-lg bg-muted/20 mt-6">
              ...
              <p className="text-muted-foreground mb-4">
                Para gerar planos nutricionais, primeiro adicione um perfil para seu pet.
              </p>
            ...
            <div className="text-center p-6 border rounded-md bg-muted/50 mt-6">
              <Image 
                ...
                height={100} 
                className="mx-auto mb-4 rounded-full opacity-80"
                data-ai-hint="sad pet food"
              />
              <p className="text-foreground mb-2">Você não possui pets com status "ativo".</p>
              <p className="text-muted-foreground">
                Planos nutricionais só podem ser gerados para pets ativos. Verifique o status dos seus pets em "Meus Pets".
              </p>
            </div>
          ) : (
          ...
          {userPets.length > 0 && activeUserPets.length < userPets.length && (
            <p className="text-sm text-muted-foreground text-center mt-4">
              Pets com status diferente de "ativo" não são listados aqui para geração de planos nutricionais.
            </p>
          )}
        </CardContent>
      </Card>
